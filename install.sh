...
sudo apt install apt-transport-https ufw fish apache2 php xfce4 xarchiver firefox mesa-utils git  pv nmap nano apt-utils dialog terminator autocutsel dbus-x11 dbus  perl p7zip unzip zip curl tar git python3 python3-pip net-tools openssl tigervnc-standalone-server tigervnc-xorg-extension -y
export HOME="$(pwd)"
export DISPLAY=":0"
cd $HOME 2> /dev/null
